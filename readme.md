...
* **Focus:** Fundamental OS manipulation, file system utilities, network commands, permission tables, and user control systems[cite: 3].
* **Key Features:** Fuzzy searching mechanics, immediate category tabs/filters, copy-on-click shortcuts, and deep descriptions for everyday terminal processes[cite: 3].

### 2. 📜 Scripting & Advanced Concepts (`Bash Scripting Reference.html`)
* **Focus:** Structural paradigms of shell code design including arrays, conditional tests, text stream processors (`sed`, `awk`, `grep`), function execution scopes, expansion tokens, and debugging parameters[cite: 3].
* **Key Features:** Embedded syntactical templates, detailed code snippets, and structured technical data schemas[cite: 3].

### 3. ⏰ Automation & Cron Management (`Bash Automation & Cron Jobs.html`)
* **Focus:** Enterprise pipeline deployment, background process control (`nohup`, `screen`, `tmux`), log auditing setups, environment preservation, error handling parameters, and highly customizable cron expression metrics[cite: 3].
* **Key Features:** Visual expression charts, conditional execution tables, and modular automated job patterns[cite: 3].

### 4. 🔀 Git & GitHub Master Hub (`git and github.html`)
* **Focus:** Distributed version control engineering, commit staging architectures, advanced branch merging/rebasing pipelines, remote configuration patterns, stash management, and GitHub collaboration strategies[cite: 3].
* **Key Features:** Visual staging lifecycle walkthroughs, instant snippet copy states, and dynamic status query parameters[cite: 3].

### 5. 🐘 PostgreSQL Reference (`postgres.html`)
* **Focus:** Relational database architecture, advanced SQL querying, index strategies, transaction isolation levels, user role management, schema migrations, and backup/restore administration workflows[cite: 3].
* **Key Features:** Comprehensive database syntax blueprints, interactive query reference patterns, performance optimization checklists, and instant copy states for complex administration commands[cite: 3].

### 6. 🔴 Redis Command Cheatsheet (`redis.html`)
